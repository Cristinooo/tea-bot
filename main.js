...
async function runWorker(workerData) {
  const { itemData, accountIndex, proxy, hasIDAPI = null, authInfos, action } = workerData;
  const to = new ClientAPI(itemData, accountIndex, proxy, hasIDAPI, authInfos);
  try {
    // 修正：每次都建立 provider/wallet，确保 provider 不为 null
    const { provider, wallet } = await connectToNetwork(proxy, itemData.privateKey);
    if (!provider) {
      to.log("Failed to connect to network. Exiting...", "error");
      parentPort.postMessage({ accountIndex, error: "No provider" });
      if (!isMainThread) parentPort.postMessage("taskComplete");
      return;
    }
    // 重新赋值 wallet，保证 provider 正确
    to.wallet = wallet;

    if (action === "2") {
      await to.handleStaking(wallet);
    } else if (action === "4") {
      await to.handleWithdrawing(wallet);
    }
...
async function main() {
  ...
  while (true) {
    ...
    await sleep(3);
    console.log(`=============${new Date().toLocaleString()} | 完成本轮 Stake/Withdraw`.magenta);
    showBanner();
    await sleep(1);
    // 无限循环
  }
